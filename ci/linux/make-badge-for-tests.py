...
def main():
  ...
  try:
    ...
    tests_count = testsuite.attrib['tests']
    failures_count = testsuite.attrib['failures']
    skipped_count = testsuite.attrib['skipped']
  ...
  success_count = tests_count - failures_count - skipped_count
  print("Found " + str(tests_count) + " tests: " + str(success_count) + " success, " + str(failures_count) + " failures and " + str(skipped_count) + " skipped tests.")
  ...
  if badge_level == LEVEL_SUCCESS:
    ...
    badge_message = str(failures_count) + " failed"
  #if skipped_count > 0:
  #  badge_message = badge_message + ", " + str(skipped_count) + " disabled"
  ...
  print("Creating " + badge_label + " badge: " + badge_namedLogo + ", " + getColorFromLevel(badge_level).lower() + ", " + badge_message)
  
  relative_path = "unittests.json"
  ...
  try:
    ...
    err.msg = "Failed to save unittests.json."
    #raise
    print(err.msg)
    sys.exit(1);
